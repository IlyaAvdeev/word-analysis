...
import one.avdeev.InvalidInput
...
class OjegovRepository : PanacheRepository<OjegovWord> {
    ...
    fun findWord(wordSize: Int,
        ...
        if (presentLetters.isNotEmpty()) {
            if (presentLetters.size != wordSize) {
                throw InvalidInput("Размер определяемого слова $wordSize не совпадает с длиной переданного слова", presentLetters)
            }
        }
        ...
        if (presentLetters.isNotEmpty()) {
            val predicateWordLikePattern = criteriaBuilder.like(
                root.get("word"),
                presentLetters.map { if (it == "?") "_" else it }.joinToString("")
            )
